...
class TOC_Customer {
     ...
     * @return void
     */
    public function set_is_logged_on($state)
    {
    ...
    /**
     * set the store credit of customer
     *
     * @access public
     * @param int
     * @return void
     */
    public function set_store_credit($credit)
    {
    	$this->data['customers_credits'] = $credit;
    }
    
    // --------------------------------------------------------------------
    
    /**
     * set the customer data with the data stored in the database
     *
     * @access public
     * @param int
     * @return void
     */
    public function set_customer_data($customer_id)
    {
    	//init customer data
    	$this->data = array();
    	
    	//load customer model to the customer data
    	$this->ci->load->model('customer_model');
    	$customer = $this->ci->customer_model->get_customer_data($customer_id);
    	
    	if ($customer !== NULL)
    	{
    		$this->set_is_logged_on(TRUE);
    		$this->set_id($customer_id);
    		$this->set_customer_group_id($customer['customers_groups_id']);
    		$this->set_gender($customer['customers_gender']);
    		$this->set_firstname($customer['customers_firstname']);
    		$this->set_lastname($customer['customers_lastname']);
    		$this->set_email_address($customer['customers_email_address']);
    		$this->set_customer_group_discount($customer['customers_groups_discount']);
    		$this->set_store_credit($customer['customers_credits']);
    		
    		if ( (is_numeric($customer['customers_default_address_id'])) && ($customer['customers_default_address_id'] > 0) )
    		{
    			if ($customer['entry_country_id'] > 0 && $customer['entry_zone_id']  > 0)
    			{
    				$this->set_country($customer['entry_country_id']);
    				$this->set_zone($customer['entry_zone_id']);
    				$this->set_default_address($customer['customers_default_address_id']);
    			}
    		}
    	}
    	...
    	elseif ($this->ci->session->userdata('customer_data') !== NULL)
    	{
    		$this->reset();
    	}
    }
