...
class TOC_Product 
{
	...
	public function update_stock($orders_id, $orders_products_id, $products_id, $products_quantity)
	{
		...
		return $this->ci->product_model->update_stock($orders_id, $orders_products_id, $products_id, $products_quantity);
	}
	...
	function get_discount_group()
	{
		...
		$discount_group = $this->product_model->get_discount_group($quantity_discount_groups_id, $customers_groups_id);
		...
		{
			$discount_group = $this->product_model->get_discount_group($quantity_discount_groups_id);
		}
	...
	/**
	 * Get quantity of simple product or variant product from database
	 *
	 * @access public
	 * @return void
	 */
	public function get_quantity_from_db()
	{
		...
		{
			$quantity = $this->ci->product_model->get_quantity($this->data['products_id']);
		...
		if ( $this->has_variants() && ! isset($this->data['default_variant']['quantity']) )
		{
			...
			{
				$quantity = $this->ci->product_model->get_quantity($variant['variants_id'], TRUE);

				if ($quantity !== NULL)
				{
					$this->data['variants'][$id]['quantity'] = $quantity;
						
					//default variant
					if ($variant['is_default'] == '1')
					{
						$this->data['default_variant']['quantity'] = $quantity;
					}
				}
			}
		}
	...
	{
		$this->ci->product_model->increment_counter();
	}
...
/* Location: ./system/library/product.php */