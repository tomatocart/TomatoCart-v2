...
class TOC_Product {
	...
	public function get_quantity_from_db()
	{
		...
		{
			$quantity = $this->ci->products_model->get_quantity($this->data['products_id']);
		...
		if ( $this->has_variants() && ! isset($this->data['default_variant']['quantity']) )
		{
			...
			{
				$quantity = $this->ci->products_model->get_quantity($variant['variants_id'], TRUE);
	...
	function get_quantity($products_id_string = '') {
		...
		if (is_numeric(strpos($products_id_string,'#'))) {
			...
			$quantity = $this->data['products_quantity'];
