...
class Product_Model extends CI_Model
{
     ...
     * @access public
     * @param $orders_id int
     * @param $orders_products_id int
     * @param $products_id int
     * @param $products_quantity int
     * @return bool
     */
    public function update_stock($orders_id, $orders_products_id, $products_id, $products_quantity)
    {
    	//error flag
    	$error = FALSE;
    	...
    	if (STOCK_LIMITED == '1')
    	{
    		//get the orders products variants
    		$result = $this->db
    		...
    		//deal with stock of products variants
    		if ($result->num_rows() > 0)
    		{
    			$variants = array();
    			foreach ($result->result_array() as $variant)
    			{
    			...
    			if ($result->num_rows() > 0)
    			{
    				$row_variant = $result->row_array();
    				$stock_left = $row_variant['products_quantity'] - $products_quantity;
    
    				//save the stock left
    				$this->db->update('products_variants', array('products_quantity' => $stock_left), array('products_variants_id' => $products_variants_id));
    
    				if ($this->db->trans_status() === FALSE)
    				{
    					$error = TRUE;
    				}
    				//Out of stock email alerts
    				else
    				{
    					if ($stock_left < 1 && STOCK_EMAIL_ALERT == '1')
    					{
    						$variants_group = $this->product->get_variants();
    
    						foreach ($variants_group as $group_id => $group)
    						{
    							if ($group['variants_id'] == $products_variants_id)
    							{
    								$variants_name = implode(',', array_values($group['groups_name']));
    								break;
    							}
    						}
    
    						//send email
    						//TOC_Product::outstock_alert($this->product->get_title(), $stock_left, $variants_name);
    					}
    				}
    			}
    		...
    		if ($error === FALSE)
    		{
    			...
    			if ($result->num_rows() > 0)
    			{
    				$row_product = $result->row_array();
    				$stock_left = $row_product['products_quantity'] - $products_quantity;
    
    				//save stock
    				$this->db->update('products', array('products_quantity' => $stock_left), array('products_id' => $products_id));
    				...
    				else
    				{
    					...
    					{
    						//send email
    						//TOC_Product::outstock_alert($row_product['products_name'], $stock_left);
    					}
    				}
    			}
    	...
    	if ($error === FALSE)
    	{
    		$this->db->update('products', array('products_ordered' => $products_quantity));
    
    		$this->db->simple_query('update ' . $this->db->dbprefix('tablename') . 'set products_ordered = products_ordered + ' . (int)$products_quantity . ' where products_id = ' . (int)$products_id);
     ...
     * @return bool
     */
    public function delete_orders_products($orders_products_id)
    {
    	//error flag
    	$error = FALSE;
    	...
    	//delete orders products variants
    	$this->db->delete('orders_products_variants', array('orders_products_id' => $orders_products_id));
    	...
    	{
    		$this->db->delete('orders_products', array('orders_products_id' => $orders_products_id));
    			
    		//ignore the downloadable product and gift certificate now
...
/* Location: ./system/models/product_model.php */