...
Ext.define('Toc.orders.OrdersGrid', {
	...
	constructor: function(config) {
		...
		config.columns =[
			...
			{header: '<?php echo lang('table_heading_status'); ?>', dataIndex: 'orders_status_name', align: 'center', width: 120},
			{
				...
				items: [{
					iconCls: 'icon-action icon-order-pdf-record',
					tooltip: '<?php echo lang('tip_print_order');?>',
					handler: function(grid, rowIndex, colIndex) {
						var rec = grid.getStore().getAt(rowIndex);
	
						this.onPrintOrder(rec);
					},
					scope: this
				},
				{
	...
	onBatchDelete: function() {
		...
		if (selections.length > 0) {
			...
			Ext.each(selections, function(item) {
				...
				this.fireEvent('batchdelete', {
					'ordersIds': keys, 'orders': orders});
		}else {
	...
	onPrintOrder: function(record) {
    	this.openWin('<?php echo site_url("pdf/print_order"); ?>/' + record.get('orders_id'), 900, 500);
  	},
